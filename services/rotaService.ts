...
export async function createSwapRequest(payload: SwapInput): Promise<RotaSwapRequest | null> {
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(T_SWAPS, 'POST', [payload], 'return=representation');
...
export async function decideSwapRequest(
    ...
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(`${T_SWAPS}?id=eq.${id}`, 'PATCH', {
        status,
        decided_by: decidedBy,
        decided_at: new Date().toISOString(),
    }, 'return=representation');
...
export async function publishShiftsInRange(
    ...
    const res = await supabaseFetch(path, 'PATCH', {
        published: true,
...
export async function unpublishShift(id: number): Promise<RotaShift | null> {
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(`${T_SHIFTS}?id=eq.${id}`, 'PATCH', {
        published: false,
        published_at: null,
        published_by: null,
    }, 'return=representation');
...
export async function releaseToOpen(shiftId: number): Promise<RotaShift | null> {
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(`${T_SHIFTS}?id=eq.${shiftId}`, 'PATCH', {
        user_id: null,
        claimed_by: null,
        claimed_at: null,
    }, 'return=representation');
...
export async function acknowledgeShift(shiftId: number, userId: string): Promise<RotaShiftAck | null> {
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(T_ACKS, 'POST', [{
        shift_id: shiftId,
        user_id: userId,
...
export async function saveBlockedDate(input: BlockedDateInput & { id?: number }): Promise<RotaBlockedDate | null> {
    ...
    if (input.id) {
        const { id, ...rest } = input;
        const res = await supabaseFetch(`${T_BLOCKED}?id=eq.${id}`, 'PATCH', rest, 'return=representation');
        const rows = await readJson<RotaBlockedDate[]>(res);
        return rows[0] || null;
    }
    const res = await supabaseFetch(T_BLOCKED, 'POST', [input], 'return=representation');
    const rows = await readJson<RotaBlockedDate[]>(res);
    return rows[0] || null;
}
...
export async function deleteBlockedDate(id: number): Promise<void> {
    if (!isSupabaseReady()) return;
    await supabaseFetch(`${T_BLOCKED}?id=eq.${id}`, 'DELETE');
}
...
export async function addToilEntry(input: ToilInput): Promise<RotaToilEntry | null> {
    if (!isSupabaseReady()) return null;
    const res = await supabaseFetch(T_TOIL, 'POST', [input], 'return=representation');
...
export async function deleteToilEntry(id: number): Promise<void> {
    if (!isSupabaseReady()) return;
    await supabaseFetch(`${T_TOIL}?id=eq.${id}`, 'DELETE');
}
...
export async function appendAudit(entry: AuditInput): Promise<void> {
    if (!isSupabaseReady()) return;
    try {
        await supabaseFetch(T_AUDIT, 'POST', [{
            ...entry,
