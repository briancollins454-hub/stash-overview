 ...
 *
 * Lives under `api/lib/` so Vercel bundles it with `api/users.ts`. Re-exported from
 * `constants/tabPermissions.ts` for the React app.
 *
 * When adding a new page: add it here, wire `validTabs` + nav in App.tsx, then deploy.
 */
