...
const isReadyToShip = (o: UnifiedOrder): boolean => {
    ...
    // Shopify fulfilled = actually left. Deco "Shipped" often means production done, not parcel sent.
    if (fs === 'fulfilled' || fs === 'refunded') return false;
    return !!(
        o.isStockDispatchReady ||
        (o.decoJobId && (o.eligibleCount ?? 0) > 0 && o.completionPercentage === 100)
    );
};
        ...
        <main className="flex-1 p-2 sm:p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto w-full">
            ...
            {activeTab === 'stock-take' && (
              <Suspense fallback={<div className="flex justify-center p-20"><Loader2 className="w-8 h-8 text-indigo-500 animate-spin" /></div>}>
                <ErrorBoundary fallbackTitle="Stock take">
                  <StockTakeScanner
                    ...
                    currentUser={{
                      ...
                      displayName: isCustomUser ? (customUserData?.displayName || customUserData?.username || '') : (user?.displayName || ''),
                    }}
                    onCommitStock={next => updatePhysicalStock(() => next)}
                    onUpdateReferenceProducts={updateReferenceProducts}
                  />
                </ErrorBoundary>
              </Suspense>
            )}
