...
// Single source of truth for priority rules. Adjust thresholds & scores here.
interface PriorityThreshold { days: number; score: number; label: string }
interface PriorityRule { status: string; metric: 'days_since_ordered' | 'days_overdue'; thresholds: PriorityThreshold[] }
...
interface PriorityResult { job: DecoJob; score: number; matchedRules: string[]; reason: string; urgency: Urgency }

function calculatePriority(job: DecoJob, now: Date): PriorityResult {
  let score = 0;
...
const URGENCY_STYLE: Record<Urgency, { dot: string; pill: string; pulse: string }> = {
  critical: { dot: 'bg-red-500/20 text-red-400',    pill: 'bg-red-500/10 text-red-400',    pulse: ' animate-pulse' },
  high:     { dot: 'bg-orange-500/20 text-orange-400', pill: 'bg-orange-500/10 text-orange-400', pulse: '' },
  medium:   { dot: 'bg-amber-500/20 text-amber-400',  pill: 'bg-amber-500/10 text-amber-400',  pulse: '' },
  low:      { dot: 'bg-blue-500/20 text-blue-400',   pill: 'bg-blue-500/10 text-blue-400',   pulse: '' },
};
