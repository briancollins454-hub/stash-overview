...
const ShippedNotInvoiced: React.FC<Props> = ({ decoJobs, isDark, settings, onNavigateToOrder, currentUserEmail }) => {
  ...
  // the heavier "go ask Deco for the last 60 days" path so the button can show
  // a different label / spinner state and we can guard against double-clicks.
  ...
  // Click-to-refresh now means "pull fresh data from Deco, merge into the
  // shared cache, then reload" — not just re-read the same stale row from
  // Supabase. Mirrors the FinancialDashboard's incrementalSync but kept
  // local so a user on this page doesn't have to hop tabs to see freshly
  // dispatched orders. Lookback = 60 days; anything older almost never
  // changes status and reloading 6 years every click would be silly.
  const pullFreshFromDeco = useCallback(async () => {
    ...
    try {
      ...
      sinceCutoff.setDate(sinceCutoff.getDate() - 60);

      // 1. Fetch fresh from Deco (last 60 days). fetchDecoFinancials uses
      //    YYYY-01-01 as its cutoff, so over-fetch slightly and trim below.
      ...
      // 2. Read whatever's already in the shared cache so we can merge
      //    rather than clobber 6 years of history with the last 60 days.
      ...
      } catch { /* first-run / cache empty — merge starts from [] */ }

      // 3. Merge: recent pull wins on overlap, untouched older rows pass
      //    through, brand-new rows get appended. Same shape as the cron.
      ...
      // 4. Persist back using the lean projection FinancialDashboard uses
      //    so the cached row shape stays identical regardless of who wrote
      //    it (this page, the Finance tab, or the nightly cron).
      ...
      // 5. Update local state directly — no need to round-trip back to
      //    Supabase since we already have the merged dataset in hand.
      setAllJobs(merged);
      setLastSynced(syncedAt);
      setNoCache(false);
      // Mark that yes, we have a sense of the cutoff (used only for
      //  the "freshly dispatched in the last 60d" tooltip below).
      void sinceCutoff;
    } catch (e: any) {
        ...
        <div>
          ...
          <p className={`text-sm ${textSecondary} mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5`}>
            {(() => {
              ...
              if (!lastSynced) return <span>Jobs shipped with outstanding balance but no invoice sent</span>;
              ...
              const stale = ageMs > 4 * 60 * 60 * 1000;   // > 4h = amber
              const veryStale = ageMs > 24 * 60 * 60 * 1000; // > 24h = red
              ...
              return (
                <>
                  ...
                  {(stale || veryStale) && (
                    <span className="text-[11px] text-amber-500">— click "Pull from Deco" to bring in newly dispatched orders</span>
                  )}
                </>
              );
            })()}
